...
import Proj2.Plane.Passenger.Passenger;
...
public class Plane extends Thread {
     ...
     * @param zoneTDPass ZoneTransferDeparture monitor (with the Passenger's Interface)
     ...
     * @param zoneEnExPass ZoneEntryExit monitor (with the Passenger's Interface)
     * @param zoneGeneralPass General monitor (with the Passenger's Interface)
     * @param zoneTAPass ZoneTransferArrival monitor (with the Passenger's Interface)
    ...
    /**
     * Simulates the coming and going of multiple airplanes
     * in the airport, each with some passengers and some bags; 
     * Is responsible for starting the Passenger Threads as well
     * as resetting zones between planes.
     *
     */
    @Override
    public void run() {
        ...
        for (int i = 0; i < nAvioes; i++) {
            ...
            for (int j = 0; j < nPassageiros; j++) {
                ...
                people[j] = new Passenger(j, zoneAPass, zoneLugPass, zoneEnExPass, zoneTAPass, zoneTDPass, malas[j], journeyOver, zoneGeneralPass, i);
            }
